...
async function handleConfirm() {
  ...
  try {
    ...
    const importedCount = itemsToImport.filter(i => !i.skip).length;
    const skippedCount = itemsToImport.filter(i => i.skip).length;
    notifyHarvestIntegration(importedCount, skippedCount);
...
<template>
  <Dialog :open="open" @update:open="closeModal">
    <DialogContent
      class="max-w-4xl max-h-[90vh] flex flex-col p-0 overflow-hidden bg-[#FDFCF8] border-emerald-100 rounded-[2.5rem] shadow-2xl">
      <!-- Header -->
      ...
      <div v-if="step === 'upload'" class="p-8 flex flex-col space-y-8 flex-1 overflow-auto">
        <div
          ...
          <div
            class="w-20 h-20 bg-emerald-50 text-emerald-700 rounded-3xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform shadow-sm">
          ...
          <div class="flex flex-col items-center gap-6">
            ...
            <a href="http://localhost:8000/samples/transactions_template.csv" download
          ...
          <div class="flex gap-4">
            ...
            <div class="text-sm">
              ...
              <p class="text-emerald-900/60 font-light italic leading-relaxed">
                A single file may contain both Income and Expense vectors. Simply tag each row appropriately in the
                'type' column for proper classification.
              </p>
            </div>
          </div>
        </div>
      ...
      <div v-else class="flex-1 overflow-hidden flex flex-col">
        <div class="px-8 py-4 border-b border-emerald-50 bg-emerald-50/20 flex items-center justify-between">
          <div class="flex items-center gap-4">
            <button @click="step = 'upload'"
              class="p-2 hover:bg-emerald-50 rounded-full transition-colors text-emerald-950/40 hover:text-emerald-950">
          ...
          <div class="text-xs font-bold uppercase tracking-widest text-rose-800">
            Skipping <span class="text-rose-950 font-black">{{previewData.filter((i: CsvImportItem) => i.skip).length
            }}</span> items
          </div>
        </div>
          ...
          <table class="w-full text-left">
            <thead>
              <tr class="border-b border-emerald-50/50">
                ...
                <th class="px-6 py-4 text-right text-[10px] font-bold text-emerald-800 uppercase tracking-widest">Value
                </th>
            ...
            <tbody class="divide-y divide-emerald-50/20">
              <tr v-for="item in previewData" :key="item.id" :class="[
                'transition-all duration-300 group',
                item.skip ? 'opacity-40 grayscale bg-stone-50/50' : 'hover:bg-white/80'
              ]">
                <td class="px-6 py-4">
                  <div class="flex items-center justify-center">
                    <input type="checkbox" :checked="!item.skip" @change="toggleSkip(item.id)"
                ...
                <td class="px-6 py-4">
                  <div>
                    ...
                    <div class="flex items-center gap-2">
                      <span
                        class="px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-tighter bg-emerald-50 text-emerald-700 border border-emerald-100/50">
                ...
                <td class="px-6 py-4">
                  <div v-if="item.is_duplicate"
                    class="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-50 text-amber-700 rounded-full border border-amber-100">
                    <AlertTriangle class="w-3.5 h-3.5" />
                    <span class="text-[9px] font-black uppercase tracking-widest">Duplicate</span>
                  </div>
                  <div v-else-if="item.status === 'valid'"
                    class="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-50 text-emerald-700 rounded-full border border-emerald-100">
                    <CheckCircle2 class="w-3.5 h-3.5" />
                    <span class="text-[9px] font-black uppercase tracking-widest">Pristine</span>
                  </div>
                  <div v-else
                    class="inline-flex items-center gap-1.5 px-3 py-1 bg-rose-50 text-rose-700 rounded-full border border-rose-100"
                    :title="item.message">
        ...
        <DialogFooter class="p-8 border-t border-emerald-50 bg-white/40 sm:justify-between items-center gap-6">
          ...
          <Button @click="handleConfirm" :disabled="isConfirming || previewData.every((i: CsvImportItem) => i.skip)"
