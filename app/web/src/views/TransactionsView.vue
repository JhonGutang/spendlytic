...
const { notifySeedSown, notifySeedRefined, notifySeedUprooted } = useBotanicalFeedback();
...
async function handleSubmit() {
  ...
  try {
    ...
    if (editingTransaction.value) {
      ...
      const category = categoryStore.categories.find(c => c.id === data.category_id)?.name || 'Unknown';
      notifySeedRefined(category);
    } else {
      await transactionStore.createTransaction(data);
      const category = categoryStore.categories.find(c => c.id === data.category_id)?.name || 'Unknown';
      notifySeedSown(category);
    }
  ...
  <div class="min-h-screen animate-in fade-in duration-1000 slide-in-from-bottom-2 font-inter pb-20">
    ...
    <div class="space-y-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
      ...
      <div class="flex flex-col md:flex-row md:items-start justify-between gap-8">
        <div>
          <h1 class="text-4xl md:text-5xl font-serif font-medium text-emerald-950 tracking-tight transition-all">
        ...
        <div class="flex flex-col gap-3 min-w-[320px]">
          <div class="flex items-center gap-3">
            <Button variant="outline" @click="openRuleEvaluation"
      ...
      <div class="flex flex-col lg:flex-row items-center justify-between gap-4">
        <div class="flex items-center gap-4 overflow-x-auto pb-2 lg:pb-0 w-full lg:w-auto">
          <div
            class="px-4 py-2 bg-white/60 backdrop-blur-sm border border-emerald-100/50 rounded-2xl flex items-center gap-3">
        ...
        <Button variant="outline" @click="showFilters = !showFilters" :class="[
          ...
          <CardContent class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6">
              <div class="space-y-2">
                ...
                <Select :model-value="transactionStore.queryParams.type || 'all'"
              ...
              <div class="space-y-2">
                ...
                <Select :model-value="transactionStore.queryParams.category_id?.toString() || 'all'"
                  ...
                  <SelectContent>
                    ...
                    <SelectItem v-for="cat in categoryStore.categories" :key="cat.id" :value="cat.id.toString()">
                      {{ cat.name }}
                    </SelectItem>
                  </SelectContent>
                </Select>
              ...
              <div class="space-y-2">
                ...
                <Input type="date" class="rounded-xl border-stone-200 bg-white"
              ...
              <div class="space-y-2">
                ...
                <div class="relative">
                  ...
                  <Input type="number" placeholder="0.00" class="rounded-xl border-stone-200 pl-7 bg-white"
              ...
              <div class="space-y-2">
                ...
                <div class="relative">
                  <div class="absolute left-3 top-1/2 -translate-y-1/2 text-emerald-900/30 text-xs">₱</div>
                  <Input type="number" placeholder="999..." class="rounded-xl border-stone-200 pl-7 bg-white"
        ...
        <CardContent class="p-0">
          ...
          <div v-else-if="transactionStore.transactions.length === 0" class="p-24 text-center">
            <div class="max-w-xs mx-auto">
              <div
                class="w-16 h-16 bg-emerald-50 text-emerald-200 rounded-full flex items-center justify-center mx-auto mb-6">
                <Search class="w-8 h-8" />
              </div>
              <h3 class="text-xl font-serif text-emerald-950 mb-2">No activity found</h3>
              <p class="text-emerald-900/50 font-light mb-6">We couldn't find any transactions matching your current
                filters.</p>
              <Button variant="outline"
                class="rounded-full border-stone-200 text-emerald-800 font-bold uppercase tracking-widest text-[10px] h-11 px-6 hover:bg-emerald-50 transition-all"
          ...
          <div v-else>
              ...
              <table class="w-full text-left">
                <thead>
                  <tr class="border-b border-emerald-100/50 bg-emerald-50/20">
                    ...
                    <th class="px-8 py-4 text-[10px] font-bold text-emerald-800 uppercase tracking-widest">Category &
                      Notes</th>
                    <th class="px-8 py-4 text-right text-[10px] font-bold text-emerald-800 uppercase tracking-widest">
                      Amount</th>
                    <th
                      class="px-8 py-4 text-right text-[10px] font-bold text-emerald-800 uppercase tracking-widest pr-10">
                      Actions</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-emerald-50/30">
                  <tr v-for="transaction in transactionStore.transactions" :key="transaction.id"
                    ...
                    <td class="px-8 py-5">
                      ...
                      <p class="text-[10px] text-emerald-600 font-bold uppercase tracking-tight mt-0.5">Time recorded
                      </p>
                    </td>
                    <td class="px-8 py-5">
                      <div>
                        <span
                          class="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-white/80 border border-emerald-100/50 text-emerald-900 mb-1">
                          {{ transaction.category?.name }}
                        </span>
                        <p class="text-sm text-emerald-950/60 italic font-light max-w-sm truncate">{{
            ...
            <!-- Pagination -->
            <div v-if="transactionStore.paginationMeta.last_page > 1"
              class="px-8 py-6 border-t border-emerald-100/50 flex flex-col md:flex-row items-center justify-between gap-4">
              <p class="text-[10px] uppercase font-bold tracking-widest text-emerald-700">
                Viewing <span class="text-emerald-950 font-black">{{ transactionStore.paginationMeta.total > 0 ?
                  (transactionStore.paginationMeta.current_page - 1) * transactionStore.paginationMeta.per_page + 1 : 0
                }}</span>
                to <span class="text-emerald-950 font-black">{{ Math.min(transactionStore.paginationMeta.current_page *
                  transactionStore.paginationMeta.per_page, transactionStore.paginationMeta.total) }}</span>
              ...
              <div class="flex items-center gap-2">
                ...
                <div class="flex items-center gap-1">
                  <Button v-for="page in transactionStore.paginationMeta.last_page" :key="page" variant="ghost"
                    size="sm" :class="[
      ...
      <DialogContent class="sm:max-w-md bg-[#FDFCF8] border-emerald-100 rounded-[2rem]">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-6 pt-4">
          ...
          <div class="flex p-1 bg-emerald-50/50 rounded-2xl border border-emerald-100/50">
            <button type="button" @click="formData.type = 'income'; formData.category_id = ''" :class="[
            ...
            <button type="button" @click="formData.type = 'expense'; formData.category_id = ''" :class="[
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="space-y-2">
              ...
              <Select v-model:model-value="formData.category_id" required>
                ...
                <SelectContent>
                  <SelectItem v-for="category in filteredCategories" :key="category.id" :value="category.id.toString()">
                    {{ category.name }}
                  </SelectItem>
                </SelectContent>
              </Select>
            ...
            <div class="space-y-2">
              ...
              <Input v-model="formData.date" type="date" required class="rounded-xl border-stone-200 h-11 bg-white"
          ...
          <div class="space-y-2">
            ...
            <div class="relative">
              ...
              <Input v-model="formData.amount" type="number" step="0.01" min="0.01" required
                class="rounded-xl border-stone-200 pl-8 h-12 text-lg font-serif bg-white" placeholder="0.00" />
            </div>
          </div>
          ...
          <div class="space-y-2">
            ...
            <Textarea v-model="formData.description" rows="3" class="rounded-xl border-stone-200 bg-white"
    ...
    <CsvUploadModal v-model:open="showImportModal" @success="transactionStore.fetchTransactions" />
  </div>
</template>
