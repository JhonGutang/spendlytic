...
- **Summary Cards**: Real-time display with **Month-over-Month Trends**:
  - **Total Income**: Sum of all recorded income transactions in ₱ (with % change vs previous month).
  - **Total Expenses**: Sum of all recorded expense transactions in ₱ (with % change vs previous month).
  - **Net Balance**: The difference between income and expenses (with % change vs previous month).
- **Financial Health Widget**:
  - **Dynamic Score**: Real-time health score (0-100) based on spending habits.
  - **Status Indicators**: Visual status (Excellent, Good, Fair, Poor) with color coding.
  - **Trend Analysis**: Indicates if financial health is improving or declining.
- **Active Insights**:
  - **Real-time Alerts**: top priority behavioral insights displayed directly on the dashboard.
  - **Actionable Feedback**: Immediate suggestions based on recent logic triggers.
- **Data Visualization**:
